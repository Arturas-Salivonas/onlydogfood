...
import { DogSearchBar } from '@/components/ui/DogSearchBar';
...
export default function DogFoodPage() {
  ...
  useEffect(() => {
    ...
    if (urlCategory || urlLifeStage || urlBreedSize || urlPriceRange) {
      setFilters(prev => ({
        ...prev,
        category: urlCategory || prev.category,
        lifeStage: urlLifeStage || prev.lifeStage,
        breedSize: urlBreedSize || prev.breedSize,
        priceRange: urlPriceRange || prev.priceRange,
      }));
    }
  }, [searchParams]);
                  ...
                  <LazyLoad fallback={<ProductGridSkeleton count={6} />}>
                    <div className="space-y-4">
                      {data?.data?.map((product) => {
                        ...
                        <div
                          key={product.id}
                          className="rounded-lg transition-all duration-200 overflow-hidden bg-[var(--color-background-card)] border border-[var(--color-border)] hover:border-[var(--color-trust)] shadow-[var(--shadow-small)] hover:shadow-[var(--shadow-medium)]"
                        >
                          <div className="flex">
                          {/* Product Image */}
                          <div className="w-40 h-40 flex-shrink-0 relative bg-[var(--color-background-neutral)] p-4">
                            <Image
                          ...
                          <div className="flex-1 p-5">
                            <div className="flex items-start justify-between mb-3">
                              <div className="flex-1">
                                ...
                                {product.brand?.slug ? (
                                  <Link
                                    ...
                                    className="text-xs font-semibold mb-1.5 tracking-wide text-[var(--color-trust)] hover:underline flex items-center gap-1.5 w-fit"
                                  ...
                                  <p className="text-xs font-semibold mb-1.5 tracking-wide flex items-center gap-1.5 text-[var(--color-trust)]">
                                    <span>{product.brand?.name}</span>
                                    {product.brand?.country_of_origin && (
                                      <span
                                        className="inline-block"
                                ...
                                {/* Product Name */}
                                <h3 className="font-bold text-lg mb-2 line-clamp-2 transition-colors text-[var(--color-text-primary)] hover:text-[var(--color-trust)]">
                                  {product.name}
                                </h3>
                              ...
                              {/* ODF Score Badge - Top Right */}
                              <div className="center-text">
                                <div
                                  className="px-4 py-2 rounded-lg font-bold text-2xl shadow-[var(--shadow-medium)] transition-all"
                                  style={{
                              ...
                              <div className="flex flex-col">
                                {product.price_per_kg_gbp ? (
                                  <>
                                    ...
                                    ≈  {formatPrice((product.price_per_kg_gbp * 0.15))} <span className="text-sm font-normal text-[var(--color-text-secondary)]">/ per meal</span>
                                    </span>

                                  </>
                                ) : (
                                  <span className="text-lg font-semibold text-[var(--color-text-secondary)]">Price TBA</span>
                                )}
                              </div>
                              <Link
                                href={`/dog-food/${product.slug}`}
                                className="flex items-center gap-2 text-sm font-bold text-[var(--color-trust)] hover:underline"
                              >

  ...
  React.useEffect(() => {
    const fetchSelectedProducts = async () => {
      ...
      try {
        ...
        logger.error('Error fetching selected products:', error);
      } finally {
    ...
    if (productsParam) {
      ...
      if (slugs.length > 0) {
        ...
        fetch('/api/products?' + new URLSearchParams({ slugs: slugs.join(','), limit: '3' }))
          ...
          .catch(err => logger.error('Error loading products from URL:', err));
      }
    }
    setUrlParsed(true);
                    ...
                    {product ? (
                      <>
                        ...
                        <div className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-bold text-white ${getScoreColor(product.overall_score || 0)}`}>
                          {Math.round(product.overall_score || 0)}<span className="text-[10px] opacity-75">/100</span>
                        </div>
